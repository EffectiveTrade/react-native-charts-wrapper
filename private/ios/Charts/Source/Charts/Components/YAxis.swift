...
open class YAxis: AxisBase
{
    ...
    /// max calculated width
    private var _lastMaxWidth = CGFloat(0)
    ...
    @objc open func requiredSize() -> CGSize
    {
        ...
        size.width = max(_lastMaxWidth, max(minWidth, min(size.width, maxWidth > 0.0 ? maxWidth : size.width)))

        if _lastMaxWidth < size.width {
            _lastMaxWidth = size.width
        }
