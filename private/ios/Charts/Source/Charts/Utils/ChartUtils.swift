...
open class ChartUtils
{
    ...
    open class func drawTextInBox(context: CGContext, text: String, point: CGPoint, align: NSTextAlignment, bg: CGColor, padding: CGFloat, radius: CGFloat, attributes: [NSAttributedString.Key : Any]?)
            {
                var point = point
                ...
                let rect = CGRect(x: point.x - padding, y: point.y - padding, width: textWidth + 2 * padding, height: textHeight + 2 * padding)
                let clipPath = UIBezierPath(roundedRect: rect, cornerRadius: radius).cgPath
