            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mTouchMode == DRAG) {
                    ...
                    if (mChart.isDragEnabled()) {
                        ...
                        boolean shouldDrag = !mChart.isFullyZoomedOut() || !mChart.hasNoDragOffset();
                        float distanceX = Math.abs(event.getX() - mTouchStartPoint.x);
                        float distanceY = Math.abs(event.getY() - mTouchStartPoint.y);
                        boolean isMoveByX = distanceX > distanceY;
                        boolean isMoveByY = distanceY >= distanceX;
                        boolean canDragX = mChart.isDragXEnabled() && !mChart.isFullyZoomedOutX();
                        boolean canDragY = mChart.isDragYEnabled() && !mChart.isFullyZoomedOutY();
//                        boolean isMoveUp = distanceY > 0;
//                        boolean isMoveDown = distanceY < 0;
//                        boolean canMoveUp = ???;
                        boolean isDragAttemptX = isMoveByX && canDragX;
                        boolean isDragAttemptY = isMoveByY && canDragY;
                        shouldDrag = shouldDrag && (isDragAttemptX || isDragAttemptY);
                        ...
                        if (shouldDrag) {
                            ...
                            mTouchMode = DRAG;
                        } else if (mChart.isHighlightPerDragEnabled() && isDragAttemptX) {
                            mLastGesture = ChartGesture.DRAG;
                            mTouchMode = HIGHLIGHT;
                            mChart.disableScroll();
                            performHighlightDrag(event);
                        } else if (isDragAttemptY) {
                            needIgnoreEvent = true;
                        }
                    }
        ...
        return !needIgnoreEvent; // indicate event was handled
    }
    ...
    public void onLongPress(MotionEvent e) {
        ...
        if (mTouchMode == NONE && mLastGesture == ChartGesture.NONE) {
