...
public class YAxisRenderer extends AxisRenderer {
    ...
    protected void drawYLabels(Canvas c, float fixedPosition, float[] positions, float offset) {
        ...
        ArrayList<Float> limitPositions = new ArrayList();
        if (limitLines != null && limitLines.size() > 0) {
          for (int i = 0; i < limitLines.size(); i++) {
            LimitLine l = limitLines.get(i);
            if (!l.isEnabled())
              continue;
            if (l.getLabelPosition() != LimitLine.LimitLabelPosition.AXIS)
              continue;
            pts[1] = l.getLimit();
            mTrans.pointValuesToPixel(pts);
            limitPositions.add(new Float(pts[1]));
          }
        }
        ...
        for (int i = from; i < to; i++) {
            ...
            isLimitLabel = false;
            for (int k = 0; k < limitPositions.size(); k++) {
              float limitY = limitPositions.get(k).floatValue();
              float min = limitY - (2 * textHeigth);
              float max = limitY + (2 * textHeigth);
              if ((y > min) && (y < max)) {
                isLimitLabel = true;
              }
            }
            ...
            if (mYAxis.getLabelBgEnabled()) {
              ...
              RectF rectF = new RectF(x - padding - alignOffset, y - textHeigth - padding, x + textWidth - alignOffset + padding, y + 2 * padding + 2);
      ...
      for (int i = 0; i < limitLines.size(); i++) {
        ...
        if (position != LimitLine.LimitLabelPosition.AXIS)
          continue;

        float y = limitPositions.get(i).floatValue() + offset;
        if (y > maxY || y < minY) {
          continue;
        }
        ...
        if (label != null && !label.equals("")) {
          ...
          mLimitLinePaint.setTypeface(mYAxis.getTypeface());
          mLimitLinePaint.setStrokeWidth(0.5f);
          mLimitLinePaint.setTextSize(l.getTextSize());
          float textWidth = mLimitLinePaint.measureText(label);
          float textHeigth = mLimitLinePaint.getTextSize();

          float x = fixedPosition;

          if (l.getLabelBgEnabled()) {
            if (l.getLabelBgEnabled()) {
              bgPaint.setColor(l.getLabelBgColor());
              bgPaint.setStyle(Paint.Style.FILL);
            }
            float radius = l.getLabelRadius();
            float alignOffset = 0;
            if (labelPosition == YAxisLabelPosition.OUTSIDE_CHART) {
              if (dependency == AxisDependency.LEFT) {
                alignOffset = textWidth;
        ...
        for (int i = 0; i < limitLines.size(); i++) {
            ...
            if (label != null && !label.equals("")) {
                ...
                if (position == LimitLine.LimitLabelPosition.RIGHT_TOP) {
                    ...
                    mLimitLinePaint.setTextAlign(Align.RIGHT);
                    float x = mViewPortHandler.contentRight() - xOffset;
                    float y = pts[1] - yOffset + labelLineHeight - 4;
                    if (l.getLabelBgEnabled()) {
                        float radius = l.getLabelRadius();
                        RectF rectF = new RectF(x - padding - alignOffset, y - textHeigth - padding, x + textWidth + padding - alignOffset, y + 2 * padding + 1);
                        c.drawRoundRect(rectF, radius, radius, bgPaint);
                    }
                    c.drawText(label, x, y, mLimitLinePaint);
                    ...
                    mLimitLinePaint.setTextAlign(Align.LEFT);
                    float x = mViewPortHandler.offsetLeft() + xOffset;
                    float y = pts[1] + yOffset + 6;
                    if (l.getLabelBgEnabled()) {
                        float radius = l.getLabelRadius();
                        RectF rectF = new RectF(x - padding, y - textHeigth - padding, x + textWidth + padding, y + 2 * padding + 1);
