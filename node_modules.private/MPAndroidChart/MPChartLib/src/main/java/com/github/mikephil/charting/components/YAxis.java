...
public class YAxis extends AxisBase {
    ...
    /**
     * The last calculated max width
     */
    private float mLastMaxWidth = 0.f;
    ...
    public float getRequiredWidthSpace(Paint p) {
        ...
        width = Math.max(mLastMaxWidth, Math.max(minWidth, Math.min(width, maxWidth > 0.0 ? maxWidth : width)));

        if (mLastMaxWidth < width)
            mLastMaxWidth = width;
