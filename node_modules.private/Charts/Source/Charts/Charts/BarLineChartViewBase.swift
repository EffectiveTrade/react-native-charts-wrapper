...
open class BarLineChartViewBase: ChartViewBase, BarLineScatterCandleBubbleChartDataProvider, NSUIGestureRecognizerDelegate
{
    ...
    internal var _longPressRecognizer: NSUILongPressGestureRecognizer!
    ...
    internal override func initialize()
    {
        ...
        _longPressRecognizer = NSUILongPressGestureRecognizer(target: self, action: #selector(longPressGestureRecognized(_:)))
    ...
    @objc private func hideMarker() {
        highlightValue(nil, callDelegate: true)
    }
    
    var endLongPressTimer: Timer?;
    
    @objc private func longPressGestureRecognized(_ recognizer: NSUILongPressGestureRecognizer)
    {
        if (recognizer.state == NSUIGestureRecognizerState.began) {
            if (endLongPressTimer != nil) {
                endLongPressTimer!.invalidate();
                endLongPressTimer = nil;
            }
        }
        ...
        if (recognizer.state == NSUIGestureRecognizerState.ended) {
            endLongPressTimer = Timer.scheduledTimer(timeInterval: 2.0, target: self, selector: #selector(hideMarker), userInfo: nil, repeats: false);
        }
    }
    ...
    @objc open var highlightPerDragEnabled = false
